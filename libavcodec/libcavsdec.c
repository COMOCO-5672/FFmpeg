...
static void set_frame_props(AVSContext *h, AVFrame *frame)
{
    ...
    frame->interlaced_frame = h->b_interlaced;
    frame->top_field_first  = h->b_interlaced;
    frame->key_frame = h->param.output_type == 0;
...
static int cavs_decode_seq_header(AVSContext *h)
{
    ...
    h->cur->interlaced_frame = h->b_interlaced;
    h->cur->top_field_first  = h->b_interlaced;
...
const FFCodec ff_cavs_lib_decoder = {
    .p.name           = "cavs",    
